...
public class CircleAnimation extends RelativeLayout {
    ...
    public CircleAnimation(Context context) {
        super(context);
        mContext = context;
        mRect = new RectF(Utils.dip2px(context, 20), Utils.dip2px(context, 20), Utils.dip2px(context, 220), Utils.dip2px(context, 220));
    }
        ...
        private Runnable drawRunnable = new Runnable() {
            ...
            public void run() {
                if (mDrawingAngle >= mEndAngle) { //绘制已完成
                    ...
                    mSeq++;
                    //间隔若干时间后,再次启动绘制任务
                    handler.postDelayed(drawRunnable, mInterval - mSeq / mFactor);
                    invalidate(); //立即触发视图的onDraw方法
                }
            }
        };
