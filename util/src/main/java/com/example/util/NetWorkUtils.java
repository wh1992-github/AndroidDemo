...
public class NetWorkUtils {
    ...
    public void getNetWorkState() {
        ...
        LogUtil.i(TAG, "isConnected: ethernet state = " + state);
        //WiFi网络的网络状态
        state = mConnectivityManager.getNetworkInfo(ConnectivityManager.TYPE_WIFI).getState();
        LogUtil.i(TAG, "isConnected: wifi state = " + state);
        //Mobile网络的状态,电视上运行报错
        state = mConnectivityManager.getNetworkInfo(ConnectivityManager.TYPE_MOBILE).getState();
        LogUtil.i(TAG, "isConnected: mobile state = " + state);

        //获取当前活跃的网络信息
        NetworkInfo info = mConnectivityManager.getActiveNetworkInfo();
        if (info != null) {
            //网络类型
            LogUtil.i(TAG, "getNetWorkState: type = " + info.getType());
            //网络类型名称
            LogUtil.i(TAG, "getNetWorkState: typename = " + info.getTypeName());
            //网络状态
            LogUtil.i(TAG, "getNetWorkState: " + info.getState());
        }
    }
