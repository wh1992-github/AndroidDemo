...
import com.example.group.databinding.ActivityScrollAlipayBinding;
...
public class ScrollAlipayActivity extends AppCompatActivity implements OnOffsetChangedListener {
    private ActivityScrollAlipayBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityScrollAlipayBinding.inflate(getLayoutInflater());
        setContentView(binding.getRoot());
        //获取默认的蓝色遮罩颜色
        mMaskColor = getResources().getColor(R.color.blue_dark);
        //从布局文件中获取名叫rv_content的循环视图
        RecyclerView rv_content = binding.rvContent;
        ...
        AppBarLayout abl_bar = binding.ablBar;
        //从布局文件中获取伸展之后的工具栏视图
        tl_expand = binding.tlExpand.getRoot();
        //从布局文件中获取收缩之后的工具栏视图
        tl_collapse = binding.tlCollapse.getRoot();
        //从布局文件中获取伸展之后的工具栏遮罩视图
        v_expand_mask = binding.tlExpand.vExpandMask;
        //从布局文件中获取收缩之后的工具栏遮罩视图
        v_collapse_mask = binding.tlCollapse.vCollapseMask;
        //从布局文件中获取生活频道的遮罩视图
        v_pay_mask = binding.lifePay.vPayMask;
