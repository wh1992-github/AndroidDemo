...
public class RxJavaActivity02 extends AppCompatActivity implements View.OnClickListener {
    ...
    private void map() {
        ...
        Observable.just(1, 2, 3, 4, 5)
                ...
                .subscribe(new Consumer<Integer>() {//订阅
                    @Override
                    public void accept(Integer integer) {//接受事件结果，是处理后的结果
                        Log.i(TAG, "map:accept = " + integer);
                    }
                });
    }
    ...
    private void flatMap() {
        Observable.create(new ObservableOnSubscribe<String>() {
            ...
            public ObservableSource<String> apply(@NotNull String s) {
                List<String> list = new ArrayList<>();
                Log.i(TAG, "flatMap:apply = 事件" + s);
    ...
    private void concatMap() {
        Observable.create(new ObservableOnSubscribe<String>() {
            ...
            public ObservableSource<String> apply(@NotNull String s) {
                ...
                Log.i(TAG, "concatMap:apply = 事件" + s);
                for (int j = 0; j < 2; j++) {
        ...
        Observable.just("A", "B", "C", "D", "E")//这里演示发送5个事件
                ...
                .subscribe(new Observer<List<String>>() {
                    ...
                    public void onSubscribe(@NotNull Disposable d) {
                        Log.i(TAG, "buffer:onSubscribe = 订阅");
                    }

                    @Override
                    public void onNext(@NotNull List<String> strings) {
                        Log.i(TAG, "buffer:onNext = 缓存事件数:" + strings.size());
                        for (int j = 0; j < strings.size(); j++) {
                            Log.i(TAG, "buffer:子事件 = " + strings.get(j));
                        }
                    }

                    @Override
                    public void onError(@NotNull Throwable e) {
                        Log.i(TAG, "buffer:onError = " + e.getMessage());
                    }

                    @Override
                    public void onComplete() {
                        Log.i(TAG, "buffer:onComplete = ");
                    }
                });
    }
    ...
    private void concatAndConcatArray() {
        ...
        Observable.concatArray(Observable.just("一", "二"), Observable.just("三", "四"),
                ...
                .subscribe(new Observer<String>() {
                    @Override
                    public void onSubscribe(@NotNull Disposable d) {
                        Log.i(TAG, "concatArray:onSubscribe = 订阅");
                    }

                    @Override
                    public void onNext(@NotNull String s) {
                        Log.i(TAG, "concatArray:onNext = " + s);
                    }

                    @Override
                    public void onError(@NotNull Throwable e) {
                        Log.i(TAG, "concatArray:onError = " + e.getMessage());
                    }

                    @Override
                    public void onComplete() {
                        Log.i(TAG, "concatArray:onComplete = ");
                    }
                });
    }
    ...
    private void merge() {
        ...
        Observable<Long> observable2 = Observable.intervalRange(11, 3, 1, 1, TimeUnit.SECONDS);

        Observable.merge(observable1, observable2)
                .subscribe(new Observer<Long>() {
                    @Override
                    public void onSubscribe(@NotNull Disposable d) {
                        Log.i(TAG, "merge:onSubscribe = 订阅");
                    }

                    @Override
                    public void onNext(@NotNull Long aLong) {
                        Log.i(TAG, "merge:onNext = " + aLong);
                    }

                    @Override
                    public void onError(@NotNull Throwable e) {
                        Log.i(TAG, "merge:onError = " + e.getMessage());
                    }

                    @Override
                    public void onComplete() {
                        Log.i(TAG, "merge:onComplete = ");
                    }
                });
    }
    ...
    private void concatDelayError() {
        ...
        Observable.concat(observable1, observable2).subscribe(new Observer<String>() {
            @Override
            public void onSubscribe(@NotNull Disposable d) {
                Log.i(TAG, "concatDelayError:onSubscribe = 订阅");
            }

            @Override
            public void onNext(@NotNull String s) {
                Log.i(TAG, "concatDelayError:onNext = " + s);
            }

            @Override
            public void onError(@NotNull Throwable e) {
                Log.i(TAG, "concatDelayError:onError = " + e.getMessage());
            }

            @Override
            public void onComplete() {
                Log.i(TAG, "concatDelayError:onComplete = ");
            }
        });
    ...
    private void mergeDelayError() {
        ...
        Observable<Object> observable1 = Observable.interval(0, 1, TimeUnit.SECONDS)
                .map(new Function<Long, Object>() {
                    @Override
                    public Object apply(@NotNull Long aLong) {
                        return aLong * 2;
                    }
        ...
        Observable<Long> observable2 = Observable.interval(1, 1, TimeUnit.SECONDS)
                .map(new Function<Long, Long>() {
                    @Override
                    public Long apply(@NotNull Long aLong) {
                        return aLong * 10;
                    }
                }).take(3);
        ...
        Observable.mergeDelayError(observable1, errorObservable, observable2)
                .subscribe(new Observer<Object>() {
                    @Override
                    public void onSubscribe(@NotNull Disposable d) {
                        Log.i(TAG, "mergeDelayError:onSubscribe = 订阅");
                    }

                    @Override
                    public void onNext(@NotNull Object o) {
                        Log.i(TAG, "mergeDelayError:onNext = " + o);
                    }

                    @Override
                    public void onError(@NotNull Throwable e) {
                        Log.i(TAG, "mergeDelayError:onError = " + e.getMessage());
                    }

                    @Override
                    public void onComplete() {
                        Log.i(TAG, "mergeDelayError:onComplete = ");
                    }
                });
    ...
    private void startWithAndStartWithArray() {
        ...
        Observable.just("一", "二", "三", "四")
                ...
                .subscribe(new Observer<String>() {
                    @Override
                    public void onSubscribe(@NotNull Disposable d) {
                        Log.i(TAG, "startWith:onSubscribe = 订阅");
                    }

                    @Override
                    public void onNext(@NotNull String s) {
                        Log.i(TAG, "startWith:onNext 结果 = " + s);
                    }

                    @Override
                    public void onError(@NotNull Throwable e) {
                        Log.i(TAG, "startWith:onError = " + e.getMessage());
                    }
    ...
    private void zip() {
        ...
        Observable.zip(observable1, observable2, new BiFunction<Integer, String, String>() {
            @NotNull
            @Override
            public String apply(@NotNull Integer integer, @NotNull String str) {
                return integer + str;
            }
        }).subscribe(new Observer<String>() {
            @Override
            public void onSubscribe(@NotNull Disposable d) {
                Log.i(TAG, "zip:onSubscribe = 订阅");
            }
            ...
            @Override
            public void onError(@NotNull Throwable e) {
                Log.i(TAG, "zip:onError = " + e.getMessage());
            }

            @Override
            public void onComplete() {
                Log.i(TAG, "zip:onComplete = ");
            }
        });
    }
    ...
    private void combineLatest() {
        ...
        Observable.combineLatest(observable1, observable2, new BiFunction<Long, Long, Long>() {
            @NotNull
            @Override
            public Long apply(@NotNull Long o1, @NotNull Long o2) {
                Log.i(TAG, "combineLatest:apply: o1+o2:" + o1 + "+" + o2);
            ...
            @Override
            public void onSubscribe(@NotNull Disposable d) {
                Log.i(TAG, "combineLatest:onSubscribe = 订阅");
            }

            @Override
            public void onNext(@NotNull Long aLong) {
                Log.i(TAG, "combineLatest:onNext 合并的结果 = " + aLong);
            }

            @Override
            public void onError(@NotNull Throwable e) {
                Log.i(TAG, "combineLatest:onError = " + e.getMessage());
            }

            @Override
            public void onComplete() {
                Log.i(TAG, "combineLatest:onComplete = ");
            }
        });
    }
    ...
    private void reduce() {
        Observable.just(1, 2, 3, 4, 5)
                .reduce(new BiFunction<Integer, Integer, Integer>() {
                    @NotNull
                    @Override
                    public Integer apply(@NotNull Integer integer, @NotNull Integer integer2) {
                        Log.i(TAG, "reduce:accept 计算结果 = " + integer + "*" + integer2);
            ...
            public void accept(Integer integer) {
                Log.i(TAG, "reduce:accept 合并的结果 = " + integer);
            }
        });
    }
    ...
    private void collect() {
        ...
        Observable.just(1, 2, 3, 4, 5).collect(new Callable<ArrayList<Integer>>() {
            ...
            public void accept(ArrayList<Integer> list, Integer integer) {
                Log.i(TAG, "collect:accept 加入容器的数据 = " + integer);
                list.add(integer);
            }
        }).subscribe(new Consumer<ArrayList<Integer>>() {
            @Override
            public void accept(ArrayList<Integer> integers) {
                Log.i(TAG, "collect:accept 最后结果 = " + integers);
            }
        });
    }
