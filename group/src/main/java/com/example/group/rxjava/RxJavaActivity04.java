...
public class RxJavaActivity04 extends AppCompatActivity implements View.OnClickListener {
    ...
    private void filter() {
        Observable.just(1, 2, 3, 4, 5, 6, 7, 8, 9)
                .filter(new Predicate<Integer>() {
                    @Override
                    public boolean test(@NotNull Integer integer) {
                        //条件：能否被2整除
                        return integer % 2 == 0;
                    }
            ...
            public void onSubscribe(@NotNull Disposable d) {
                Log.i(TAG, "filter：onSubscribe = 订阅");
            }

            @Override
            public void onNext(@NotNull Integer integer) {
                Log.i(TAG, "filter：onNext = " + integer);
            }

            @Override
            public void onError(@NotNull Throwable e) {
                Log.i(TAG, "filter：onError = " + e.getMessage());
            }

            @Override
            public void onComplete() {
                Log.i(TAG, "filter：onComplete = ");
            }
        });
    }
    ...
    private void ofType() {
        Observable.just(1, "大闸蟹", true, 0.23f, 5L, new Goods("商品名"))
            ...
            @Override
            public void onSubscribe(@NotNull Disposable d) {
                Log.i(TAG, "ofType：onSubscribe = 订阅");
            }

            @Override
            public void onNext(@NotNull Goods goods) {
                Log.i(TAG, "ofType：onNext = " + goods.getName());
            }

            @Override
            public void onError(@NotNull Throwable e) {
                Log.i(TAG, "ofType：onError = " + e.getMessage());
            }

            @Override
            public void onComplete() {
                Log.i(TAG, "ofType：onComplete = ");
            }
        });
    }
        ...
        Observable.just(1, 2, 3, 4, 5, 6, 7, 8, 9)
                ...
                .subscribe(new Consumer<Integer>() {
                    ...
                    public void accept(Integer integer) {
                        Log.i(TAG, "elementAt：accept = " + integer);
                    }
                });
    }
    ...
    private void distinct() {
        Observable.just(1, 2, 2, 3, 4, 4, 4)
                ...
                .subscribe(new Observer<Integer>() {
                    @Override
                    public void onSubscribe(@NotNull Disposable d) {
                        Log.i(TAG, "distinct：onSubscribe = 订阅");
                    }

                    @Override
                    public void onNext(@NotNull Integer integer) {
                        Log.i(TAG, "distinct：onNext = " + integer);
                    }

                    @Override
                    public void onError(@NotNull Throwable e) {
                        Log.i(TAG, "distinct：onError = " + e.getMessage());
                    }
        ...
        Observable.create(new ObservableOnSubscribe<Integer>() {
            ...
            public void subscribe(@NotNull ObservableEmitter<Integer> e) {
                ...
                .subscribe(new Observer<Integer>() {
                    @Override
                    public void onSubscribe(@NotNull Disposable d) {
                        Log.i(TAG, "debounce：onSubscribe = 订阅");
                    }

                    @Override
                    public void onNext(@NotNull Integer integer) {
                        Log.i(TAG, "debounce：onNext = " + integer);
                    }

                    @Override
                    public void onError(@NotNull Throwable e) {
                        Log.i(TAG, "debounce：onError = " + e.getMessage());
                    }
    ...
    private void first() {
        Observable.just(1, 2, 3)
                ...
                .subscribe(new Consumer<Integer>() {
                    @Override
                    public void accept(Integer integer) {
                        Log.i(TAG, "first：accept = " + integer);
                    }
                });
    }
    ...
    private void skip() {
        Observable.just(1, 2, 3, 4, 5, 6, 7)
                ...
                .subscribe(new Observer<Integer>() {
                    @Override
                    public void onSubscribe(@NotNull Disposable d) {
                        Log.i(TAG, "skip：onSubscribe = 订阅");
                    }

                    @Override
                    public void onNext(@NotNull Integer integer) {
                        Log.i(TAG, "skip：onNext = " + integer);
                    }

                    @Override
                    public void onError(@NotNull Throwable e) {
                        Log.i(TAG, "skip：onError = " + e.getMessage());
                    }
    ...
    private void take() {
        Observable.just(1, 2, 3, 4, 5, 6, 7)
                ...
                .subscribe(new Observer<Integer>() {
                    @Override
                    public void onSubscribe(@NotNull Disposable d) {
                        Log.i(TAG, "take：onSubscribe = 订阅");
                    }

                    @Override
                    public void onNext(@NotNull Integer integer) {
                        Log.i(TAG, "take：onNext = " + integer);
                    }

                    @Override
                    public void onError(@NotNull Throwable e) {
                        Log.i(TAG, "take：onError = " + e.getMessage());
                    }

                    @Override
                    public void onComplete() {
                        Log.i(TAG, "take：onComplete = ");
                    }
                });
    }
    ...
    private void groupBy() {
        ...
        Observable.interval(1, TimeUnit.SECONDS)
            ...
            @Override
            public void onSubscribe(@NotNull Disposable d) {
                Log.i(TAG, "groupBy：onSubscribe = 订阅");
            }

            @Override
            public void onNext(@NotNull GroupedObservable<Object, Long> objectLongGroupedObservable) {
                Log.i(TAG, "groupBy：onNext = key:" + objectLongGroupedObservable.getKey());
            }

            @Override
            public void onError(@NotNull Throwable e) {
                Log.i(TAG, "groupBy：onError = " + e.getMessage());
            }
    ...
    private void cast() {
        Observable.just(1, 2, 3, 4, 5, 6)
                ...
                .subscribe(new Observer<Integer>() {
                    @Override
                    public void onSubscribe(@NotNull Disposable d) {
                        Log.i(TAG, "cast：onSubscribe = 订阅");
                    }

                    @Override
                    public void onNext(@NotNull Integer integer) {
                        Log.i(TAG, "cast：onNext = " + integer);
                    }

                    @Override
                    public void onError(@NotNull Throwable e) {
                        Log.i(TAG, "cast：onError = " + e.getMessage());
                    }

                    @Override
                    public void onComplete() {
                        Log.i(TAG, "cast：onComplete = ");
                    }
                });
    }
    ...
    private void scan() {
        Observable.just(1, 2, 3)
                .scan(new BiFunction<Integer, Integer, Integer>() {
                    @NotNull
                    @Override
                    public Integer apply(@NotNull Integer sum, @NotNull Integer integer) {
                        //sum是上次计算的记过，integer是本次计算的参数
                        Log.i(TAG, "scan：apply = sum + integer = " + sum + " + " + integer);
                        return sum + integer;
                    }
                }).subscribe(new Observer<Integer>() {
            @Override
            public void onSubscribe(@NotNull Disposable d) {
                Log.i(TAG, "scan：onSubscribe = 订阅");
            }
            ...
            @Override
            public void onError(@NotNull Throwable e) {
                Log.i(TAG, "scan：onError = " + e.getMessage());
            }

            @Override
            public void onComplete() {
                Log.i(TAG, "scan：onComplete = ");
            }
        });
    }
    ...
    private void join() {
        ...
        observable1.join(observable2, new Function<Long, ObservableSource<Long>>() {
            @Override
            public ObservableSource<Long> apply(@NotNull Long aLong) {
            ...
            @NotNull
            @Override
            public Long apply(@NotNull Long aLong, @NotNull Long aLong2) {
                //合并逻辑
                Log.i(TAG, "BiFunction：apply = aLong1 + aLong2：" + aLong + " + " + aLong2);
                return aLong + aLong2;
            }
        }).subscribe(new Observer<Long>() {
            @Override
            public void onSubscribe(@NotNull Disposable d) {
                Log.i(TAG, "join：onSubscribe = 订阅");
            }

            @Override
            public void onNext(@NotNull Long aLong) {
                Log.i(TAG, "join：onNext = " + aLong);
            }

            @Override
            public void onError(@NotNull Throwable e) {
                Log.i(TAG, "join：onError = " + e.getMessage());
            }

            @Override
            public void onComplete() {
                Log.i(TAG, "join：onComplete = ");
            }
        });
    }
    ...
    private void groupJoin() {
        ...
        observable1.groupJoin(observable2, new Function<Long, ObservableSource<Long>>() {
            ...
            @NotNull
            @Override
            public Observable<Long> apply(@NotNull final Long aLong, @NotNull Observable<Long> longObservable) {
                return longObservable.map(new Function<Long, Long>() {
                    @Override
                    public Long apply(@NotNull Long aLong2) {
                        //合并逻辑
                        Log.i(TAG, "BiFunction：apply = aLong1 + aLong2：" + aLong + " + " + aLong2);
                        return aLong + aLong2;
                    }
            ...
            @Override
            public void onSubscribe(@NotNull Disposable d) {
                Log.i(TAG, "groupJoin：onSubscribe = 订阅");
            }

            @Override
            public void onNext(@NotNull Observable<Long> longObservable) {
                longObservable.subscribe(new Consumer<Long>() {
                    @Override
                    public void accept(Long aLong) {
                        Log.i(TAG, "groupJoin：onNext = " + aLong);
                    }
                });
            }
            ...
            @Override
            public void onError(@NotNull Throwable e) {
                Log.i(TAG, "groupJoin：onError = " + e.getMessage());
            }

            @Override
            public void onComplete() {
                Log.i(TAG, "groupJoin：onComplete = ");
            }
        });
    }
}
