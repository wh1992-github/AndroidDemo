...
convertToColor(const GUTF8String &s) {
    ...
    if (s.length()) {
        ...
        if (endpos < 0) {
            G_THROW((ERR_MSG("XMLAnno.bad_color")
                            "\t")+s);
        }
    }
    return retval;
...
//           DjVuPrintMessage("Coords (%d,%d)\n",x,y);
                ...
                if (shape == "default") {
                    ...
                    G_THROW((ERR_MSG("XMLAnno.unknown_shape")
                                    "\t")+shape);
                }
                if (a) {
                    GPosition pos;
                    ...
                    if ((pos = args.contains("bordertype"))) {
                        ...
                        if ((pos = typeMap.contains(b))) {
                            ...
                            G_THROW((ERR_MSG("XMLAnno.unknown_border")
                                            "\t")+b);
                        }
                    }
    ...
    GCriticalSectionLock lock(&xmlparser_lock);
    {
        ...
        if (pos) {
            ...
            if (!doc->wait_for_complete_init()) {
                G_THROW((ERR_MSG("XMLAnno.fail_init")
                                "\t")+url.get_string());
            }
...
//         DjVuPrintErrorUTF8("Ignoring %s Object tag\n",mimetype);
            ...
            const GURL url = GURL::UTF8(args[datapos],
...
lt_XMLParser::Impl::parse_anno(
    ...
    GP<lt_XMLTags> map;
    {
        ...
        if (usemappos) {
            ...
            if (!mappos) {
                G_THROW((ERR_MSG("XMLAnno.map_find")
                                "\t")+mapname);
            } else {
                map = Maps[mappos];
            }
