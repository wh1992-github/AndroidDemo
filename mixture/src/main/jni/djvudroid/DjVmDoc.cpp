...
DjVmDoc::delete_file(const GUTF8String &id) {
    ...
    if (!data.contains(id))
        G_THROW(GUTF8String(ERR_MSG("DjVmDoc.cant_delete")
                        "\t") + id);
...
DjVmDoc::get_data(const GUTF8String &id) const {
    ...
    if (!data.contains(id, pos))
        G_THROW(GUTF8String(ERR_MSG("DjVmDoc.cant_find")
                        "\t") + id);
    ...
    G_TRY
            {
                ...
                if (size < 0 || size > 0x7fffffff)
                    G_THROW(ERR_MSG("DjVmDoc.not_IFF")
                                    "\t" + id);
            }
        G_CATCH_ALL
            {
...
DjVmDoc::write(const GP<ByteStream> &gstr,
    ...
    for (pos = files_list; pos; ++pos) {
        ...
        if (!data_pos)
            G_THROW(ERR_MSG("DjVmDoc.no_data")
                            "\t" + file->get_load_name());
        if (do_rename) {
