...
DjVuDocument::start_init(
    ...
    if (!url.is_empty()) {
        ...
        DEBUG_PRINT("Requesting data for: %s",
                    init_url.get_string(true).getUTF82Native().getbuf(128));
        if (init_data_pool) {
        ...
        if (!init_data_pool) {
            G_THROW(ERR_MSG("DjVuDocument.fail_URL")
                            "\t" + init_url.get_string());
        }
    }
                ...
                G_TRY
                        {
                            ...
                            if (!exc.cmp_cause(ByteStream::EndOfFile) && th->verbose_eof)
                                get_portcaster()->notify_error(th,
                                                               ERR_MSG("DjVuDocument.init_eof"));
                            else if (!exc.cmp_cause(DataPool::Stop))
                                get_portcaster()->notify_status(th,
                                                                ERR_MSG("DjVuDocument.stopped"));
                            else
            ...
            G_TRY
                    {
                        ...
                        if (!new_url.is_empty()) {
                            ...
                            if (f->id_type == UnnamedFile::ID)
                                ...
                                msg = ERR_MSG("DjVuDocument.miss_page_num") "\t" +
                                      GUTF8String(f->page_num);
                            G_THROW(msg);
                        }
                        ++pos;
                    }
        ...
        switch (doc_type) {
            case OLD_BUNDLED: {
                if (flags & DOC_DIR_KNOWN) {
                    ...
                    if (!file) {
                        G_THROW(ERR_MSG("DjVuDocument.file_outside")
                                        "\t" + url.fname());
                    }
            ...
            case INDIRECT: {
                ...
                if (flags & DOC_DIR_KNOWN)
                    if (doc_type == INDIRECT && !djvm_dir->id_to_file(url.fname()))
                        G_THROW(ERR_MSG("DjVuDocument.URL_outside2")
                                        "\t" + url.get_string());
                        ...
                        G_TRY
                                {
                                    ...
                                    GUTF8String emsg =
                                            ERR_MSG("DjVuDocument.exclude_page") "\t" + (i + 1);
