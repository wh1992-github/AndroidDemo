...
//The name may contain brackets to specify the chunk number
{
    ...
    if (!pos) {
        G_THROW(ERR_MSG("GIFFManager.no_chunk")
                        "\t" + short_name + "\t" + GUTF8String(number) + "\t" + get_name());
    }
}
...
//pos=-1 means to append the chunk
{
    ...
    if (parent_name.length() && parent_name[0] == '.') {
        ...
        if (!top_level->check_name(top_name))
            G_THROW(ERR_MSG("GIFFManager.wrong_name")
                            "\t" + top_name);
...
//       number=atoi((const char *)name.substr(obracket+1,cbracket-obracket-1));
            ...
            if (!cur_sec)
                G_THROW(ERR_MSG("GIFFManager.unknown")
                                "\t" + name);
        }
    } while (*end);
...
//It may also end with [] to set the chunk order number
{
    ...
    if (name[0] == '.') {
        ...
        if (next_dot < 0) {
            ...
            G_THROW(ERR_MSG("GIFFManager.wrong_name2")
                            "\t" + name.substr(1, (unsigned int) -1));
        }
    ...
    do {
        ...
        if (!cur_sec)
            G_THROW(ERR_MSG("GIFFManager.cant_find")
                            "\t" + GUTF8String(name));
    } while (*end);

    if (!start[0]) {
        G_THROW(GUTF8String(ERR_MSG("GIFFManager.malformed")
                        "\t")+name);
    }
