    ...
    if (files) {
        ...
        for (pos = files_list; pos; ++pos) {
            ...
            if (name2file.contains(file->name))
                G_THROW(ERR_MSG("DjVmDir.dupl_name")
                                "\t" + file->name);
        ...
        for (pos = files_list; pos; ++pos) {
            ...
            if (id2file.contains(file->id))
                G_THROW(ERR_MSG("DjVmDir.dupl_id")
                                "\t" + file->id);
        ...
        for (pos = files_list; pos; ++pos) {
            ...
            if (file->title.length()) {
                if (title2file.contains(file->title))
                    G_THROW(ERR_MSG("DjVmDir.dupl_title")
                                    "\t" + file->title);
...
DjVmDir::insert_file(const GP<File> &file, int pos_num) {
    ...
    if (id2file.contains(file->id))
        G_THROW(ERR_MSG("DjVmDir.dupl_id2")
                        "\t" + file->id);
    if (name2file.contains(file->name))
        G_THROW(ERR_MSG("DjVmDir.dupl_name2")
                        "\t" + file->name);
    name2file[file->name] = file;
    id2file[file->id] = file;
    if (file->title.length()) {
        if (title2file.contains(file->title))
            ...
            G_THROW(ERR_MSG("DjVmDir.dupl_title2")
                            "\t" + file->title);
        title2file[file->title] = file;
    }
...
DjVmDir::set_file_name(const GUTF8String &id, const GUTF8String &name) {
    ...
    for (pos = files_list; pos; ++pos) {
        ...
        if (file->id != id && file->name == name)
            G_THROW(ERR_MSG("DjVmDir.name_in_use")
                            "\t" + GUTF8String(name));
    }

    //Check if ID is valid
    if (!id2file.contains(id, pos))
        G_THROW(ERR_MSG("DjVmDir.no_info")
                        "\t" + GUTF8String(id));
