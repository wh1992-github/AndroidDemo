...
GLObject::operator[](int n) const {
    ...
    if (n >= list.size())
        G_THROW(ERR_MSG("DjVuAnno.too_few")
                        "\t" + name);
    int i;
    GPosition pos;
                ...
                if (obj && obj->get_list().size() == 1) {
                    ...
                    if (retval == ZOOM_UNSPEC) {
                        if (zoom[0] != 'd') {
                            ...
                            retval = zoom.substr(1,
                                                 zoom.length()).toInt(); //atoi((const char *) zoom+1);
                        }
                    }
                }
#ifndef NDEBUG
                                    ...
                                    if (el->get_type() == GLObject::LIST) {
                                        ...
                                        if (name == GMapArea::BORDER_AVIS_TAG)
                                            ...
                                            if (obj->get_type() == GLObject::SYMBOL)
                                                map_area->hilite_color = cvt_color(
                                                        obj->get_symbol(), 0xff);
                                        } else {
                                            int border_type =
                                                    name == GMapArea::NO_BORDER_TAG
                                                    ? GMapArea::NO_BORDER :
                                                    name == GMapArea::XOR_BORDER_TAG
                                                    ? GMapArea::XOR_BORDER :
