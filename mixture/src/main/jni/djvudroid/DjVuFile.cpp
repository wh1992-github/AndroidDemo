...
DjVuFile::init(const GURL &xurl, GP<DjVuPort> port) {
    ...
    if (!(data_pool = DataPool::create(pcaster->request_data(this, url))))
        G_THROW(ERR_MSG("DjVuFile.no_data")
                        "\t" + url.get_string());
    data_pool->add_trigger(-1, static_trigger_cb, this);
}
                ...
                G_TRY
                        {
                            if (!exc.cmp_cause(DataPool::Stop)) {
                                ...
                                flags.leave();
                                pcaster->notify_status(this,
                                                       GUTF8String(ERR_MSG("DjVuFile.stopped"))
...
DjVuFile::process_incl_chunk(ByteStream &str, int file_num) {
    ...
    if (incl_str.length() > 0) {
        ...
        G_ENDCATCH;
        if (!file) {
            G_THROW(ERR_MSG("DjVuFile.no_create")
                            "\t" + incl_str);
        }
        ...
        G_TRY
                {
                    ...
                    int chksize;
                    for (; (chunks_left--) &&
                           (chksize = iff.get_chunk(chkid)); last_chunk = chunks) {
                        chunks++;
                        if (chkid == "INCL") {
                            G_TRY
                                    {
                    ...
                    if (is_info(chkid) && info) {
                        ...
                        if (!processed_meta) {
                            ...
                            copy_chunks(meta, ostr);
                                                    ...
                                                    dir)) {  //Copy NDIR chunks, but never generate new ones.
                        ostr.put_chunk(chkid);
