...
ByteStream::Stdio::init(const GURL &url, const char mode[]) {
    ...
    if (url.fname() != "-") {
        ...
        if (!fp) {
            // Failed to open '%s': %s
            G_THROW(ERR_MSG("ByteStream.open_fail")
                            "\t" + url.name()
...
ByteStream::Memory::seek(long offset, int whence, bool nothrow) {
    ...
    switch (whence) {
        ...
        case SEEK_END:
            ...
            break;
        default:
            G_THROW(ERR_MSG("bad_arg")
                            "\tByteStream::Memory::seek()");      //Illegal argument in ByteStream::Memory::seek()
    }
    nwhere += offset;
    if (nwhere < 0)
        G_THROW(ERR_MSG(
                "ByteStream.seek_error2"));                          // Attempt to seek before the beginning of the file
    where = nwhere;
    return 0;
}
...
ByteStream::Static::seek(long offset, int whence, bool nothrow) {
    ...
    switch (whence) {
        ...
        case SEEK_END:
            ...
            break;
        default:
            G_THROW("bad_arg\tByteStream::Static::seek()");      // Illegal argument to ByteStream::Static::seek()
    }
    nwhere += offset;
    if (nwhere < 0)
        G_THROW(ERR_MSG(
