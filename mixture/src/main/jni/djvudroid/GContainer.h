...
public:
    ...
    /** Inserts an element before the first element of the list.
        The new element is initialized with a copy of argument #elt#. */
    void prepend(const TYPE &elt) { GListImpl<TI>::prepend(newnode((const TI &) elt)); }
...
public:
    ...
    const KTYPE &key(const GPosition &pos) const {
        return (const KTYPE &) (((typename GMapImpl<KTYPE, TI>::MNode *) (pos.check(
                (void *) this)))->key);
    }
    ...
    VTYPE &operator[](const GPosition &pos) {
        return (VTYPE &) (((typename GMapImpl<KTYPE, TI>::MNode *) (pos.check(
                (void *) this)))->val);
    }
    ...
    const VTYPE &operator[](const KTYPE &key) const {
        return (const VTYPE &) (((const typename GMapImpl<KTYPE, TI>::MNode *) (this->get_or_throw(
                key)))->val);
    }
    ...
    VTYPE &operator[](const KTYPE &key) {
        return (VTYPE &) (((typename GMapImpl<KTYPE, TI>::MNode *) (this->get_or_create(
                key)))->val);
    }
