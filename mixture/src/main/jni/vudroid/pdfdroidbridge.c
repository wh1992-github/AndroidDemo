...
jclass clazz, jlong
handle,
jfloatArray mediabox
)
{
renderpage_t *page = (renderpage_t *) handle;
jfloat *bbox = (*env)->GetPrimitiveArrayCritical(env, mediabox, 0);
if(!bbox) return;
bbox[0] = page->page->mediabox.
x0;
bbox[1] = page->page->mediabox.
y0;
bbox[2] = page->page->mediabox.
x1;
bbox[3] = page->page->mediabox.
y1;
(*env)->
ReleasePrimitiveArrayCritical(env, mediabox, bbox,
0);
}
...
y = viewbox.y0;
pixmap->
w = viewbox.x1 - viewbox.x0;
pixmap->
h = viewbox.y1 - viewbox.y0;
pixmap->
n = 4;

DEBUG("Allocating temp buffer");
pixmap->
samples = (*env)->GetDirectBufferAddress(env, tempBuffer);
if (!pixmap->samples)
{
goto
cleanup;
