...
cff_make_private_dict(CFF_SubFont subfont,
    ...
    priv->blue_shift = (FT_Int)
    cpriv->blue_shift;
    priv->blue_fuzz = (FT_Int)
    cpriv->blue_fuzz;
...
sub->
units_per_em = FT_MulDiv(sub->units_per_em,
                         top->units_per_em,
                         scaling);
}
else
...
/* set up num_faces */
cffface->
num_faces = cff->num_faces;

/* compute number of glyphs */
if ( dict->cid_registry != 0xFFFFU )
cffface->
num_glyphs = cff->charset.max_cid;
else
cffface->
num_glyphs = cff->charstrings_index.count;
...
cffface->
height = (FT_Short)(cffface->ascender - cffface->descender);
...
cffface->
family_name = cff_index_get_name(&cff->name_index,
                                 face_index);
...
goto
Skip_Unicode; /* Standard Unicode (deprecated) */
}
...
face = cffface;
cmaprec.
platform_id = 7;  /* Adobe platform id */
