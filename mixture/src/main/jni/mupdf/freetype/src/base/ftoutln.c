...
return
FT_Outline_New_Internal( library
->memory, numPoints,
...
return
FT_Outline_Done_Internal( library
->memory, outline );
}
...
return
FT_Outline_Render( library, outline,
&params );
}
...
outline->points[n].
x = v_cur.x + strength + in.x;
outline->points[n].
y = v_cur.y + strength + in.y;
...
contour < outline->contours + outline->
n_contours;
contour++,
first = last + 1
)
