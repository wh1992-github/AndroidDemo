...
FT_ASSERT((slot
->internal->flags & FT_GLYPH_OWN_BITMAP) == 0 );
}
...
if ( bsize->height < 0 )
bsize->
height = (FT_Short) - bsize->height;
if ( bsize->x_ppem < 0 )
bsize->
x_ppem = (FT_Short) - bsize->x_ppem;
if ( bsize->y_ppem < 0 )
bsize->
y_ppem = -bsize->y_ppem;
}
...
akerning->
x = FT_MulDiv(akerning->x,
              face->size->metrics.x_ppem, 25);
if ( face->size->metrics.y_ppem < 25 )
akerning->
y = FT_MulDiv(akerning->y,
...
return service->
table_info( face, table_index, tag,
&offset, length );
}
...
{
FT_ASSERT( module
->clazz && module->clazz->get_interface );
...
library->
raster_pool_size = FT_RENDER_POOL_SIZE;
