...
t1_decoder_parse_charstrings( T1_Decoder
decoder,
...
hinter->
reset( hinter
->hints, builder->current->n_points );
break;
...
Syntax_Error;
builder->
parse_state = T1_Parse_Have_Moveto;
}
break;
...
zone->
base = decoder->subrs[idx];

if ( decoder->subrs_len )
zone->
limit = zone->base + decoder->subrs_len[idx];
else
{
...
FT_LOCAL_DEF( FT_Error )
t1_decoder_parse_glyph( T1_Decoder
decoder,
FT_UInt glyph
)
{
return decoder->
parse_callback( decoder, glyph
);
}
