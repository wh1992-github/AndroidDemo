...
root->
units_per_EM = (FT_UShort)(FT_DivFix(0x10000L,
...
return
CID_Err_Ok;      /* this is a callback function; */
...
cid_read_subrs(CID_Face face) {
    ...
    for (n = 0; n < cid->num_dicts; n++, subr++) {
        ...
        for (count = 0; count <= num_subrs; count++)
            offsets[count] = cid_get_offset(&p, (FT_Byte)
        dict->sd_bytes );
