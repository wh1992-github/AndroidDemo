        ...
        if (!strcmp(buf, "BX")) {
            ...
            if (!strcmp(fz_toname(obj), "Pattern")) {
                ...
                if (!strcmp(fz_toname(obj), "DeviceGray"))
                    ...
                    if (!obj)
                        return fz_throw("cannot find colorspace resource /%s",
                                        fz_toname(csi->stack[0]));
            ...
            if (!dict)
                return fz_throw("cannot find XObject dictionary when looking for: '%s'",
                                fz_toname(csi->stack[0]));
            ...
            obj = fz_dictget(dict, csi->stack[csi->top - 1]);
            if (!obj)
                return fz_throw("cannot find shade resource: %s",
                                fz_toname(csi->stack[csi->top - 1]));
