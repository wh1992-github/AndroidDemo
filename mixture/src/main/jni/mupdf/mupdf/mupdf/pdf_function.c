...
enum {
    PSBOOL, PSINT, PSREAL, PSOPERATOR, PSBLOCK
};
...
evalpostscriptfunc(pdf_function *func, psstack *st, int codeptr) {
    ...
    while (1) {
        switch (func->u.p.code[codeptr].type) {
            case PSINT: SAFE_PUSHINT(st, func->u.p.code[codeptr++].u.i);
                break;

            case PSREAL: SAFE_PUSHREAL(st, func->u.p.code[codeptr++].u.f);
                break;
            ...
            case PSOPERATOR:
                switch (func->u.p.code[codeptr++].u.op) {
                    ...
                    case PSOATAN: SAFE_POPNUM(st, &r2);
                        SAFE_POPNUM(st, &r1);
                        SAFE_PUSHREAL(st, atan2(r1, r2) * RADIAN);
                        break;

                    case PSOBITSHIFT: SAFE_POPINT(st, &i2);
                        SAFE_POPINT(st, &i1);
                        if (i2 > 0) {
                            SAFE_PUSHINT(st, i1 << i2);
                        } else if (i2 < 0) {
                    ...
                    case PSODIV: SAFE_POPNUM(st, &r2);
                        SAFE_POPNUM(st, &r1);
                        SAFE_PUSHREAL(st, r1 / r2);
                        break;

                    case PSODUP: SAFE_COPY(st, 1);
                        break;
                    ...
                    case PSOEXP: SAFE_POPNUM(st, &r2);
                        SAFE_POPNUM(st, &r1);
                        SAFE_PUSHREAL(st, pow(r1, r2));
                        break;

                    case PSOFALSE: SAFE_PUSHBOOL(st, 0);
                        break;
                    ...
                    case PSOIDIV: SAFE_POPINT(st, &i2);
                        SAFE_POPINT(st, &i1);
                        SAFE_PUSHINT(st, i1 / i2);
                        break;

                    case PSOINDEX: SAFE_POPINT(st, &i1);
                        SAFE_INDEX(st, i1);
                        break;
                    ...
                    case PSOLN: SAFE_POPNUM(st, &r1);
                        SAFE_PUSHREAL(st, log(r1));
                        break;

                    case PSOLOG: SAFE_POPNUM(st, &r1);
                    ...
                    case PSOPOP: SAFE_POP(st);
                        break;

                    case PSOROLL: SAFE_POPINT(st, &i2);
                        SAFE_POPINT(st, &i1);
                        psroll(st, i1, i2);
                        break;
                    ...
                    case PSOSIN: SAFE_POPNUM(st, &r1);
                        SAFE_PUSHREAL(st, sin(r1 / RADIAN));
                        break;

                    case PSOSQRT: SAFE_POPNUM(st, &r1);
                    ...
                    case PSOTRUE: SAFE_PUSHBOOL(st, 1);
                        break;
                    ...
                    case PSOIFELSE: SAFE_POPBOOL(st, &b1);
                        if (b1) {
                            error = evalpostscriptfunc(func, st, codeptr + 2);
                            if (error)
                                return fz_rethrow(error, "runtime error in if-branch");
    ...
    switch (func->type) {
        case SAMPLE:
            ...
            if (error) {
                ...
                return fz_rethrow(error, "cannot load sampled function (%d %d R)", fz_tonum(ref),
                                  fz_togen(ref));
            }
            break;
        ...
        case EXPONENTIAL:
            ...
            if (error) {
                pdf_dropfunction(func);
                return fz_rethrow(error, "cannot load exponential function (%d %d R)",
                                  fz_tonum(ref), fz_togen(ref));
            }
            break;
        ...
        case STITCHING:
            ...
            if (error) {
                pdf_dropfunction(func);
                return fz_rethrow(error, "cannot load stitching function (%d %d R)", fz_tonum(ref),
                                  fz_togen(ref));
            }
            break;
        ...
        case POSTSCRIPT:
            ...
            if (error) {
                pdf_dropfunction(func);
                return fz_rethrow(error, "cannot load calculator function (%d %d R)", fz_tonum(ref),
                                  fz_togen(ref));
            }
            break;
...
pdf_evalfunction(pdf_function *func, float *in, int inlen, float *out, int outlen) {
    ...
    switch (func->type) {
        ...
        case POSTSCRIPT: {
            ...
            for (i = 0; i < func->m; ++i) SAFE_PUSHREAL(&st, in[i]);
