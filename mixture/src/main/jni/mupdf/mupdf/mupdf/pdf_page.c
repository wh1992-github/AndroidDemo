...
runone(pdf_csi *csi, pdf_xref *xref, fz_obj *rdb, fz_obj *stmref) {
    ...
    if (error)
        return fz_rethrow(error, "cannot load content stream (%d %d R)", fz_tonum(stmref),
                          fz_togen(stmref));
    ...
    if (error)
        return fz_rethrow(error, "cannot interpret content stream (%d %d R)", fz_tonum(stmref),
                          fz_togen(stmref));
...
runmany(pdf_csi *csi, pdf_xref *xref, fz_obj *rdb, fz_obj *list) {
    ...
    for (i = 0; i < fz_arraylen(list); i++) {
        ...
        if (error) {
            fz_dropbuffer(big);
            return fz_rethrow(error, "cannot load content stream part %d/%d", i + 1,
                              fz_arraylen(list));
        }
...
loadpagecontents(fz_tree **treep, pdf_xref *xref, fz_obj *rdb, fz_obj *obj) {
    ...
    if (obj && error) {
        pdf_dropcsi(csi);
        return fz_rethrow(error, "cannot interpret page contents (%d %d R)", fz_tonum(obj),
                          fz_togen(obj));
    }
