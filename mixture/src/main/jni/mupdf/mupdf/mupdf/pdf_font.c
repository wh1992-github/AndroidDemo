        ...
        if (kind == TRUETYPE) {
            ...
            if (face->charmap && face->charmap->platform_id == 3) {
                ...
                for (i = 0; i < 256; i++) {
                    ...
                    if (fterr) {
                        error = fz_throw("freetype get glyph name (gid %d): %s", etable[i],
                                         ft_errorstring(fterr));
                        goto cleanup;
                    }
                    if (ebuffer[i][0])
...
static fz_error
loadcidfont(pdf_fontdesc **fontdescp, pdf_xref *xref, fz_obj *dict, fz_obj *encoding,
            fz_obj *tounicode) {
    fz_error error;
    fz_obj *widths;
    fz_obj *descriptor;
    ...
    if (kind == TRUETYPE) {
        ...
        if (fz_isindirect(cidtogidmap)) {
            ...
            if (fterr) {
                error = fz_throw("fonterror: no unicode cmap when emulating CID font: %s",
                                 ft_errorstring(fterr));
                goto cleanup;
            }
...
pdf_loadfontdescriptor(pdf_fontdesc *fontdesc, pdf_xref *xref, fz_obj *dict, char *collection) {
    ...
    if (fz_isindirect(obj)) {
        ...
        if (error) {
            fz_catch(error,
                     "ignored error when loading embedded font, attempting to load system font");
