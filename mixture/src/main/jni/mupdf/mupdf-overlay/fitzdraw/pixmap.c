...
fz_error
fz_newpixmapwithbuffer(fz_pixmap **pixp, void *buf, int x, int y, int w, int h, int n) {
    fz_pixmap *pix;
...
fz_error
fz_newpixmapwithbufferandrect(fz_pixmap **pixp, void *buf, fz_irect r, int n) {
    return fz_newpixmapwithbuffer(pixp, buf, r.x0, r.y0, r.x1 - r.x0, r.y1 - r.y0, n);
}
...
void
fz_droppixmapwithoutbuffer(fz_pixmap *pix) {
    fz_free(pix);
}
