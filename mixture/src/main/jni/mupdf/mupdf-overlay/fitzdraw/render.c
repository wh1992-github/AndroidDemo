...
rendersolid(fz_renderer *gc, fz_solidnode *solid, fz_matrix ctm) {
    ...
    DEBUG("solid %s [%d %d %d %d];\n", solid->cs->name, gc->argb[0], gc->argb[1], gc->argb[2],
          gc->argb[3]);
...
renderpath(fz_renderer *gc, fz_pathnode *path, fz_matrix ctm) {
    ...
    if (gc->flag & FRGB) {
        DEBUG(" path rgb %d %d %d %d, %d %d %d\n", gc->argb[0], gc->argb[1], gc->argb[2],
              gc->argb[3], gc->argb[4], gc->argb[5], gc->argb[6]);
...
blendmask(fz_renderer *gc, fz_pixmap *src, fz_pixmap *msk, fz_pixmap *dst, int over) {
    ...
    if (over) {
        if (src->n == 1 && msk->n == 1 && dst->n == 1)
            ...
            fz_duff_nimon(sp, src->w * src->n, src->n, mp, msk->w * msk->n, msk->n, dp,
                          dst->w * dst->n, w, h);
        else
        ...
        if (src->n == 1 && msk->n == 1 && dst->n == 1)
            ...
            fz_duff_nimcn(sp, src->w * src->n, src->n, mp, msk->w * msk->n, msk->n, dp,
                          dst->w * dst->n, w, h);
        else
...
fz_error
fz_rendertreetopixmap(fz_pixmap **outp,
                      fz_renderer *gc, fz_tree *tree, fz_matrix ctm,
                      fz_irect bbox, int white) {
    fz_error error;
