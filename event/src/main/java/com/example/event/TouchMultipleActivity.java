...
public class TouchMultipleActivity extends AppCompatActivity {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        String desc_major = String.format(Locale.getDefault(), "主要动作发生时间：开机距离现在%02d:%02d:%02d\n%s",
                hour, minute, second, "主要动作名称是：");
        String desc_minor = "";
        //获得包括次要点在内的触摸行为
        int action = event.getAction() & MotionEvent.ACTION_MASK;
        if (action == MotionEvent.ACTION_DOWN) { //手指按下
            desc_major = String.format(Locale.getDefault(), "%s按下", desc_major);
        } else if (action == MotionEvent.ACTION_MOVE) { //手指移动
            desc_major = String.format(Locale.getDefault(), "%s移动", desc_major);
            if (isMinorPressed) {
                desc_minor = String.format(Locale.getDefault(), "%s次要动作名称是：移动", desc_minor);
            }
        } else if (action == MotionEvent.ACTION_UP) { //手指松开
            desc_major = String.format(Locale.getDefault(), "%s提起", desc_major);
        } else if (action == MotionEvent.ACTION_CANCEL) { //取消手势
            desc_major = String.format(Locale.getDefault(), "%s取消", desc_major);
        } else if (action == MotionEvent.ACTION_POINTER_DOWN) { //次要点按下
            isMinorPressed = true;
            desc_minor = String.format(Locale.getDefault(), "%s次要动作名称是：按下", desc_minor);
        } else if (action == MotionEvent.ACTION_POINTER_UP) { //次要点松开
            isMinorPressed = false;
            desc_minor = String.format(Locale.getDefault(), "%s次要动作名称是：提起", desc_minor);
        }
        desc_major = String.format(Locale.getDefault(), "%s\n主要动作发生位置是：横坐标%f,纵坐标%f",
                desc_major, event.getX(), event.getY());
        tv_touch_major.setText(desc_major);
        if (isMinorPressed || !TextUtils.isEmpty(desc_minor)) { //存在次要点触摸
            desc_minor = String.format(Locale.getDefault(), "%s\n次要动作发生位置是：横坐标%f,纵坐标%f",
